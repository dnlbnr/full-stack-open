...
router.get('/', async (request, response) => {
  ...
  const blogs = await Blog.find({ user: userId }).populate('user', { username: 1, name: 1, id: 1 })
...
router.get('/:id', async (request, response) => {
  ...
  const blog = await Blog.findOne({ _id: id, user: userId }).populate('user', { username: 1, name: 1, id: 1 })
  if (!blog) {
...
router.put('/:id', async (request, response) => {
  ...
  const result = await Blog.findByIdAndUpdate(id, data, { new:true, runValidators: true, context:'query' })
